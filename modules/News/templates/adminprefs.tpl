...
<legend>{$title_fesubmit_settings}</legend>
        <div class="warning">{$mod->Lang('warn_fesubmit_deprecated')}</div>
	<div class="pageoverflow">
		<p class="pagetext">{$mod->Lang('fesubmit_allowed')}:</p>
		<p class="pageinput">
                  <select name="{$actionid}fesubmit_allowed">{html_options options=$yesno_opts selected=$fesubmit_allowed}</select>
		</p>
	</div>
	<div class="pageoverflow">
