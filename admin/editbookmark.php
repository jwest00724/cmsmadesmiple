...
$title = "";
if (isset($_POST["title"])) $title = trim(cleanValue((string)$_POST["title"]));

$myurl = "";
if (isset($_POST["url"])) $myurl = trim(cleanValue((string)$_POST["url"]));
...
if (isset($_POST["editbookmark"])) {
    ...
    if( stripos($myurl,'script:') !== FALSE || stripos($myurl,'<script') !== FALSE ) {
        $error .= lang('missingparams');
        $validinfo = false;
    }
